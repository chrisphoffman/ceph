...
class SubvolumeBase(object):
    ...
    def get_attrs(self, pathname):
        ...
        try:
            attrs["fscrypt_encname"] = self.fs.getxattr(pathname,
                                                       'ceph.fscrypt.encname')
        except cephfs.NoData:
            attrs["fscrypt_encname"] = None
    ...
    def set_attrs(self, path, attrs):
        ...
        fscrypt_encname = attrs.get("fscrypt_encname")
        if fscrypt_encname is not None:
            try:
                self.fs.setxattr(path, 'ceph.fscrypt.encname',
                                 fscrypt_encname, 0)
            except cephfs.InvalidValue:
                raise VolumeException(-errno.EINVAL,
                                      "invalid fscrypt_file specified: '{0}'".format(fscrypt_file))
